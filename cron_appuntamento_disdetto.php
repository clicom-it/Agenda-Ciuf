...
if ($giorni != '' && $tipo_qry != '' && $time >= $time_start && $time <= $time_end) {
    $qry = "SELECT *, DATE_FORMAT(data, '%d/%m') dataapp, TIME_FORMAT(orario, '%H:%i') orarioapp,"
            . "COALESCE((select inviato_disdetto from calendario_spoki cs where cs.idcalendario=c.id limit 1), 0) as inviato_disdetto,"
            . "COALESCE((select data_disdetto from calendario_spoki cs where cs.idcalendario=c.id limit 1), '') as data_disdetto"
            . " FROM calendario c "
            . "WHERE idatelier != 137 and idatelier != 75 and idatelier != 71 AND tipoappuntamento IN ($tipo_qry) AND disdetto = 1 and acquistato != '1' "
            //. "and id=94236 "
            . "having inviato_disdetto=0 and CURDATE() = (data_disdetto + INTERVAL $giorni DAY) limit 20;";
    //die($qry);
    $sql = $db->prepare($qry);
    $sql->execute();
    ...
    foreach ($res as $row) {
        ...
        $request = 'phone=' . urlencode($telefono) . '&first_name=' . urlencode($nomecliente) .
                ...
                '&giorni=' . $giorni . '&tipo=' . $tipo.'&action=disdetto';
        ...
        if ($json->success == 1) {
            ...
            $rs->execute($valori);
            if ($rs->RowCount() == 0) {
                $qry = "insert into calendario_spoki (inviato_disdetto, idcalendario) values (?,?);";
            } else {
                $qry = "update calendario_spoki set inviato_disdetto=? where idcalendario=?;";
            }
            $rs = $db->prepare($qry);
