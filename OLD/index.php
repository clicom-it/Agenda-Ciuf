...
switch ($submit) {
    case "login":
        ...
        $sql = $db->prepare("SELECT * FROM utenti WHERE username = ? AND password = ? AND attivo = ? AND username != '' AND password != '' LIMIT 1");
        try {
...
//            if ($sessemail == "") {
            ...
            $atelier_collegati = getDati("atelier_collegati", "WHERE idatelier = " . $_SESSION['idatelier'] . "");
            ...
            if (count($atelier_collegati) > 0) {
                ...
                foreach ($atelier_collegati as $at) {
                    $_SESSION['atelier_collegati'][] = $at['idatelier2'];
                }
            }
//            } else {
    ...
    <body style="background-color: #6d6e70;" onload="$('#username').focus();">
        ...
        <div class="login">
            ...
            <form method="post" action="" name="formlogin" id="formlogin">
                <input style="background-image:url('./immagini/user.png');" type="text" name="username" id="username" placeholder="Username" class="input_login sizing required" />
