...
// =====================================================
$ch = curl_init(GESTIONALE_API_URL);
...
if ($curlError)                              syncError('Impossibile contattare il gestionale: ' . $curlError);
if ($httpCode < 200 || $httpCode >= 300)     syncError("Il gestionale ha risposto con codice HTTP {$httpCode}");
$gestionaleRaw = json_decode($response, true);
if (!is_array($gestionaleRaw))               syncError('Il gestionale non ha restituito JSON valido');
...
foreach ($gestionaleRaw as $raw) {
    ...
    // --- Verifica attivo ---
    if ((string)($raw[FIELD_ATTIVO] ?? '') !== (string)FIELD_ATTIVO_VALORE) continue;
    ...
    if (!$sede) continue;  // salta se sede mancante
    ...
    if ($dataNascitaRaw) {
        $dtNasc = DateTime::createFromFormat(FIELD_DATA_NASCITA_FORMAT, $dataNascitaRaw)
               ?: DateTime::createFromFormat('d/m/Y', $dataNascitaRaw)
               ?: DateTime::createFromFormat('d-m-Y', $dataNascitaRaw);
        if ($dtNasc) $dataNascitaMySQL = $dtNasc->format('Y-m-d');
    }
    ...
    if ($dataInizioRaw) {
        $dtInizio = DateTime::createFromFormat('Y-m-d', $dataInizioRaw)
                 ?: DateTime::createFromFormat('d/m/Y', $dataInizioRaw)
                 ?: DateTime::createFromFormat('d-m-Y', $dataInizioRaw);
        if ($dtInizio) $dataInizioMySQL = $dtInizio->format('Y-m-d');

        if ($dataFineRaw) {
            $dtFine = DateTime::createFromFormat('Y-m-d', $dataFineRaw)
                   ?: DateTime::createFromFormat('d/m/Y', $dataFineRaw)
                   ?: DateTime::createFromFormat('d-m-Y', $dataFineRaw);
            if ($dtFine) {
