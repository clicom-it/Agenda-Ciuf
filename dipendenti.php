...
switch ($submit) {
        ...
        if (isset($_POST['ore_settimana'])) {
            ...
            foreach ($_POST['data_inizio_r'] as $i => $data_inizio_r) {
                $dati_retribuzione[] = Array(
                    ...
                    'ore_settimana_r' => $_POST['ore_settimana_r'][$i]
                );
            }
            if ($rs->RowCount() > 0) {
                $qry = "update utenti_dipendenti set ore_settimana=?, stipendio_netto=?, dati_retribuzione=?, idatelier_sede=?, data_inizio=?, data_fine=? where idutente=?;";
                $valori = Array($ore_settimana, $stipendio_netto, json_encode($dati_retribuzione), $idatelier_sede, $data_inizio, $data_fine, $id);
            } else {
                $qry = "insert into utenti_dipendenti (idutente, ore_settimana, stipendio_netto, dati_retribuzione, idatelier_sede, data_inizio, data_fine) values (?,?,?,?,?,?,?);";
                $valori = Array($id, $ore_settimana, $stipendio_netto, json_encode($dati_retribuzione), $idatelier_sede, $data_inizio, $data_fine);
            }
            //var_dump($valori);
            //die($qry);
            $rs = $db->prepare($qry);
            ...
            //die();
            unset($_POST['ore_settimana'], $_POST['stipendio_netto'], $_POST['data_inizio_r'], $_POST['data_inizio'], $_POST['retribuzione'], $_POST['data_fine_r'], $_POST['data_fine'], $_POST['ore_settimana_r'], $_POST['idatelier_sede']);
        }
        //var_dump($_POST);
        $valori = Array();
    ...
    case "submitformdipendenti":
        ...
        if (isset($_POST['ore_settimana'])) {
            ...
            $idatelier_sede = $_POST['idatelier_sede'];
            unset($_POST['ore_settimana'], $_POST['stipendio_netto'], $_POST['data_inizio_r'], $_POST['data_inizio'], $_POST['retribuzione'], $_POST['data_fine_r'], $_POST['data_fine'], $_POST['idatelier_sede']);
        } else {
    ...
    <body class="colormodulo">
        ...
        <?php if (moduloattivo("dipendenti") > 0 || $_SESSION['ruolo'] == CENTRALINO || $_SESSION['ruolo'] == RISORSE_UMANE || ($_SESSION['livello'] == '5' && $_SESSION["diraff"] == 'a')) { ?>
            <div class="barra_submenu sizing">
                ...
                <?php } ?>
                <?php if ($_SESSION["livello"] == 0 || $_SESSION["livello"] == 1) { ?>
                    <li class="box_submenu sizing"><a href="dipendenti.php?op=sicurezza"><i class="fa fa-briefcase" aria-hidden="true"></i> Sicurezza</a></li>
                <?php } ?>
            </div>
