...
?>
  ...
  FROM budget_periods where year=$year
...
");
                        ...
                        <div style="width: 100%;margin: 20px 0 50px;">
                            <input type="submit" class="submit_form nopost" value="Importa dati da Google" id="import-google" />
                        </div>
                        <header>
                            <h1>Budget</h1>
...
")->fetchAll();
  ...
  FROM utenti s
  where attivo=1 and livello='5' and (diraff='d' or s.id=76) and s.id in (select store_id from store_budget_targets)
...
");
  ...
  SELECT
    ...
    COALESCE((select cognome from utenti u inner join store_budget_targets sbt on u.id=sbt.storeM_id and sbt.store_id=s.id where period_id=$period_id limit 1),'') as cognome_store
  FROM utenti s
  where attivo=1 and livello='5' and (diraff='d' or s.id=76) $qry_atelier
  ORDER BY s.nominativo ASC";
                            //echo "$qry";
                            $stores = $pdo->query($qry)->fetchAll(PDO::FETCH_ASSOC);
            ...
            <!--                                        <input type="month" id="quick" value="<?= sprintf('%04d-%02d', $selY, $selM) ?>" oninput="jumpToMonth(this.value)">-->
                                ...
                                <div class="card">
                                    ...
                                    function rowHtml(array $s, array $targets, array $actuals, array $prizes, string $CURRENCY, float $percBudget, float $percDistrict): string {
                                        ...
                                        $t4 = $targets[$sid][4] ?? null;
                                        $t5 = $targets[$sid][5] ?? null;
                                        $hasPremio50Perc = hasPremio50Perc($pdo, $selM, $selY, $sid);
                                        $act = $actuals[$sid] ?? 0.0;
                                        ...
                                        if ($premi[0] > 0) {
                                            ...
                                            while ($user = $rs->fetch(PDO::FETCH_ASSOC)) {
                                                if ($hasPremio50Perc) {
                                                    $premio_budget = $user['total_sold_amount'] * $percBudget / 2;
                                                } else {
                                                    $premio_budget = $user['total_sold_amount'] * $percBudget;
                                                }
                                                $arrChart[strtoupper(substr($user['nome'], 0, 1)) . '. ' . $user['cognome']] = $user['total_sold_amount'];
                                                echo '<p>' . strtoupper(substr($user['nome'], 0, 1)) . '. ' . $user['cognome'] . ': ' . fmtMoney($user['total_sold_amount'], $CURRENCY) . ' <span style="color:green;font-weight:bold;">Premio ' . $percBudget . '%: ' . fmtMoney($premio_budget / 100, $CURRENCY) . '</span></p>';
                                            }
                                        }
                                        echo '<p>&nbsp;</p>';
                                        if ($hasPremio50Perc) {
                                        ...
                                        if ($_SESSION['id'] == $id_district_m || $_SESSION['livello'] <= 1 || $_SESSION['ruolo'] == RISORSE_UMANE) {
                                            ...
                                            echo ((float) $premi[2] > 0 ? '<div class="prize">Premio DM: <strong>' . fmtMoney($premi[2]/$diff_premio, $CURRENCY) . '</strong></div>' : '')
                                            . ((float) $premi[1] > 0 ? '<div class="prize">Premio SM: <strong>' . fmtMoney($premi[1]/$diff_premio, $CURRENCY) . '</strong></div>' : '');
                                        } elseif ($_SESSION['id'] == $id_store_m) {
                                            echo ((float) $premi[1] > 0 ? '<div class="prize">Premio SM: <strong>' . fmtMoney($premi[1]/$diff_premio, $CURRENCY) . '</strong></div>' : '');
                                        } else {
