...
class ore extends basic {
    ...
    function aggiungi($campi, $valori, $voci) {
        ...
        if ($voci) {
            $sql = $db->prepare("INSERT INTO ore_voci (idutente, nomecognomeutente, idgiornoore, dataoredettaglio, idcomm, idvocecomm, descr, orelavorate, idprofit, ordine) VALUES (?,?,?,?,?,?,?,?,?,?)");
            /* ore lavorate commessa */
            $sql1 = $db->prepare("SELECT orelavorate FROM ore_voci WHERE idcomm = ?");
            /* update ore lavorate su commessa */
            $sql2 = $db->prepare("UPDATE commesse SET orelavorate = ? WHERE id = ?");
            /**/
            for ($i = 0; $i < count($voci); $i++) {
                $sql->execute(array($voci[$i]['idutente'], $voci[$i]['nomecognomeutente'], $idgiornoore, $voci[$i]['dataoredettaglio'], $voci[$i]['idcomm'], $voci[$i]['idvocecomm'], $voci[$i]['descr'], $voci[$i]['orelavorate'], $voci[$i]['idprofit'], $voci[$i]['ordine']));
                ...
                $sql2->execute(array($orelavoratecomm, $voci[$i]['idcomm']));
            }
        }
    }
    ...
    function aggiorna($campi, $valori, $voci) {
        ...
        if ($voci) {
            ...
            for ($i = 0; $i < count($voci); $i++) {
                ...
                $sql->execute(array($voci[$i]['idutente'], $voci[$i]['nomecognomeutente'], $this->id, $voci[$i]['dataoredettaglio'], $voci[$i]['idcomm'], $voci[$i]['idvocecomm'], $voci[$i]['descr'], $voci[$i]['orelavorate'], $voci[$i]['idprofit'], $voci[$i]['ordine']));
            }
        }
        ...
        for ($n = 0; $n < count($arraycommesseok); $n++) {
            ...
            $orelavoratecomm = sommaorelavorate($orelavorate);
            /* update ore lavorate commessa */
            $sql2->execute(array($orelavoratecomm, $arraycommesseok[$n]));
        }
    }
