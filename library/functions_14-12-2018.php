...
function getDatiCal() {
    global $db;
    $sql = $db->prepare("SELECT *, DATE_FORMAT(data, '%d/%m/%Y') as datait, TIME_FORMAT(orario, '%H:%i') as orariodb FROM calendario");
...
function meseprofit($anno, $mese, $where) {
    ...
    $sql = $db->prepare('SELECT id, tipo FROM fatture WHERE MONTH(data) = ? AND YEAR(data) = ? AND tipo IN("0","4","3") ' . $where . '');
...
function totaleprofit($anno, $where) {
    ...
    $sql = $db->prepare('SELECT id, tipo FROM fatture WHERE YEAR(data) = ? AND tipo IN ("0","3","4") ' . $where . '');
    $sql1 = $db->prepare('SELECT SUM((qta*(importo - (importo * sconto / 100)))*(100+iva) / 100) as sommaprofit, (SELECT nome FROM profit_center WHERE id = idprofit) as nomeprofit, idprofit FROM fatture_voci WHERE idfattura = ? GROUP by idprofit');
...
function moduloattivo($modulo) {
    ...
    if ($sql->fetchColumn() > 0) {
        ...
        if ((($_SESSION['livello'] == 2 || $_SESSION['livello'] == 3) && $_SESSION[$modulo] == 0)) {
            $attivo = 0;
        } else {
            $attivo = 1;
        }
...
function generaFatturaElettronica($fatturad) {
    ...
    <DatiTrasmissione>
      ...
      <CodiceDestinatario>' . ($cliente['codice_sdi'] != "" ? $cliente['codice_sdi'] : '0000000') . '</CodiceDestinatario>' .
            ($cliente['pec'] != "" && $cliente['codice_sdi'] == "" ? '<PECDestinatario>' . $cliente['pec'] . '</PECDestinatario>' : '') . '
    </DatiTrasmissione>
    <CedentePrestatore>
      <DatiAnagrafici>
        <IdFiscaleIVA>
    ...
    <CessionarioCommittente>
      <DatiAnagrafici>' .
          ...
          <IdCodice>' . $cliente['piva'] . '</IdCodice>
            ...
            '') .
            (strlen($cf) == 16 ? '<CodiceFiscale>' . strtoupper($cf) . '</CodiceFiscale>' : '') .
            '<Anagrafica>' .
            ($cliente['azienda'] != "" ? '<Denominazione>' . str_replace("&", "&amp;", $cliente['azienda']) . '</Denominazione>' :
    ...
    <DatiGenerali>
      <DatiGeneraliDocumento>
        ...
        <Numero>' . $fatturad['numero'] . '/' . $anno_fatt . '</Numero>
    ...
    /* tabella iva */
    $sql = $db->prepare("SELECT sum(qta*(importo-importo*sconto/100)) as tabimponibile, sum((qta*(importo-importo*sconto/100))*iva/100) tabimposta, iva FROM `fatture_voci` "
    ...
    foreach ($res as $row) {
        ...
        <AliquotaIVA>' . number_format($tabiva, 2, ".", "") . '</AliquotaIVA>' .
                ($tabiva == 0 ? '<Natura>N6</Natura>' : '') .
                '<ImponibileImporto>' . number_format($tabimponibile, 2, ".", "") . '</ImponibileImporto>
        <Imposta>' . number_format($tabimposta, 2, ".", "") . '</Imposta>' .
                ($tabiva == 0 ? '<RiferimentoNormativo>Reverse charge Art. 17 DPR 633/72</RiferimentoNormativo>' : '') .
                '</DatiRiepilogo>';
    }
