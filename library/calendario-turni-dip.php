...
//    $day = $date->format('w');
        ...
        $arrayore[$i]['evento'] = 0;
        foreach ($utenti as $k => $idutente_cal) {
            if ($idutente_cal == $datid['idutente']) {
                $arrayore[$i]['color'] = $colori[$k];
            }
        }
        if ($_SESSION['livello'] <= 1 || $_SESSION['ruolo'] == 2) {
            $arrayore[$i]['edit'] = 1;
        } else {
            $arrayore[$i]['edit'] = 0;
        }
...
//echo "WHERE id > 0 " . ($idutente != '' ? "and idutente = '" . $idutente . "'" : "") . " and data_cal between '" . $_GET['start'] . "' and '" . $_GET['end'] . "'" . ($idatelier != '' ? " and idatelier=$idatelier" : "") . ";";
