...
class fatture extends basic {
    ...
    function aggiungi($campi, $valori, $voci, $scadenze, $statofatt, $tipo) {
        ...
        if ($voci) {
            $sql = $db->prepare("INSERT INTO fatture_voci (idfattura, nome, descrizione, um, qta, importo, sconto, iva, idprev, idddt, idordine, idcomm, idvocecomm, idfatt, idprofit, ordine, codice_iva) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            for ($i = 0; $i < count($voci); $i++) {
                $sql->execute(array($idfattura, $voci[$i]['nome'], $voci[$i]['descrizione'], $voci[$i]['um'], $voci[$i]['qta'], $voci[$i]['importo'], $voci[$i]['sconto'], $voci[$i]['iva'], $voci[$i]['idprev'], $voci[$i]['idddt'], $voci[$i]['idordine'], $voci[$i]['idcomm'], $voci[$i]['idvocecomm'], $voci[$i]['idfatt'], $voci[$i]['idprofit'], $voci[$i]['ordine'], $voci[$i]['codice_iva']));
                /* array commesse */
    ...
    function aggiorna($campi, $valori, $voci, $scadenze, $modificascadenze, $statofatt, $tipo) {
        ...
        if ($voci) {
            ...
            for ($i = 0; $i < count($voci); $i++) {
                ...
                $arridcomm[] = $voci[$i]['idcomm'];
                /**/
                $sql->execute(array($this->id, $voci[$i]['nome'], $voci[$i]['descrizione'], $voci[$i]['um'], $voci[$i]['qta'], $voci[$i]['importo'], $voci[$i]['sconto'], $voci[$i]['iva'], $voci[$i]['idprev'], $voci[$i]['idddt'], $voci[$i]['idordine'], $voci[$i]['idcomm'], $voci[$i]['idvocecomm'], $voci[$i]['idfatt'], $voci[$i]['idprofit'], $voci[$i]['ordine'], $voci[$i]['codice_iva']));
            }
        }
