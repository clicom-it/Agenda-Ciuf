...
//$arrAtelierSartoria = Array(65, 59, 136, 140, 100, 97);
...
<script type="text/javascript" src="./js/functions-calendario.js?v=2"></script>
<form method="post" action="" id="formstep2cal" name="formstep2cal" style="font-size: 0.9em;">   
    <div class="float_primibox float_lft sizing">
        <div class="boxsx sizing">
            ...
            <div class="nomeacc sizing">Provenienza:</div>
            <select name="provenienza" id="provenienza" class="input_moduli sizing float_moduli required" placeholder="Provenienza contatto" title="Provenienza contatto">
                <?php
            ...
            <div class="chiudi"></div>
            <?php
            $pren = getDati("calendario", "where id='" . $_GET['id'] . "'");
            if ($pren[0]['acquistato'] == '1') {
                $acquistato = 'SI';
            } elseif ($pren[0]['acquistato'] == '0') {
                $acquistato = 'NO';
            } else {
                $acquistato = '';
            }
            $idatelier = $pren[0]['idatelier'];
            //echo $_SESSION["diraff"].' '.$_SESSION['livello'];
            ?>
            <?php if ($_SESSION['livello'] > 1 && $_SESSION["diraff"] != 'a') { ?>
                <textarea name="note" id="note" class="textarea_moduli_small sizing" title="Note" placeholder="Note"></textarea>
                <div class="chiudi"></div>
                <div class="sizing" style="margin:10px 0;">Disdetto: <?= ($pren[0]['disdetto'] == '1' ? 'SI' : 'NO') ?></div>
                <div class="chiudi"></div>
                <div class="sizing" style="margin:10px 0;">Il cliente ha Acquistato? <?= $acquistato ?></div>
                <div class="chiudi"></div>
                <?php if ($pren[0]['acquistato'] == '0') { ?>
                    <div class="sizing" style="margin:10px 0;">Nel caso in cui l'appuntamento non sia andato a buon fine, o non sia stato acquistato nulla, indica il motivo: <?php
                        $motivono = getDati("motivonoacq", "where id=" . $pren[0]['idnoacquisto'] . " limit 1;");
                        echo $motivono[0]['nome'];
                        ?></div>
                    <div class="chiudi"></div>
                <?php } ?>
                <div class="sizing" style="margin:10px 0;">Chi ha gestito l'appuntamento? <?php
                    $dipendente = getDati("utenti", "where id=" . $pren[0]['idutente']);
                    echo ' <b>' . $dipendente[0]['nome'] . ' ' . $dipendente[0]['cognome'] . '</b>';
                    ?></div>
                <div class="chiudi"></div>
            <?php } else { ?>
                <div class="nomeacc sizing">Disdetto:</div>
                ...
                <select name="acquistato" id="acquistato" onchange="nosartoria(this.value);"  class="input_moduli sizing float_moduli_small_25" placeholder="L'abito è stato acquistato?" title="L'abito è stato acquistato?">
                    <option value="">L'abito è stato acquistato?</option>
                    <option value="1">SI</option>
                    <option value="0">NO</option>
                </select>
                <div class="chiudi"></div>
                Nel caso in cui l'appuntamento non sia andato a buon fine, o non sia stato acquistato nulla, indica il motivo:
                <div class="chiudi" style="height: 10px;"></div>
                <select name="idnoacquisto" id="idnoacquisto"  class="input_moduli sizing float_moduli" placeholder="Motivo non acquistato" title="Motivo non acquistato">            
                    <?php
                    $motivono = getDati("motivonoacq", "ORDER BY id ");
                    echo "<option value=\"\">Seleziona il motivo</option>";
                    foreach ($motivono as $motivonod) {
                        echo "<option value=\"" . $motivonod['id'] . "\">" . $motivonod['nome'] . "</option>";
                    }
                    ?>
                </select>
                <div class="chiudi"></div>
                Chi ha gestito l'appuntamento?
                <?php
                $mostra_select_dip = false;
                if ($pren[0]['idutente'] > 0 && ($_SESSION['livello'] == 5 || $_SESSION['livello'] == 3)) {
                    $dipendente = getDati("utenti", "where id=" . $pren[0]['idutente'] . " and attivo=1 and livello='3' and ruolo in (" . ADDETTO_VENDITE . "," . DISTRICT_MANAGER . "," . FRANCHISING . ", " . STORE_MANAGER . ")");
                    if (count($dipendente) > 0) {
                        $mostra_select_dip = true;
                    }
                }
                if ($pren[0]['idutente'] > 0 && ($_SESSION['livello'] == 5 || $_SESSION['livello'] == 3) && $mostra_select_dip) {
                    $dipendente = getDati("utenti", "where id=" . $pren[0]['idutente']." and livello='3'");
                    echo ' <b>' . $dipendente[0]['nome'] . ' ' . $dipendente[0]['cognome'] . '</b>';
                } else {
                    ?>
                    <div class="chiudi" style="height: 10px;"></div>
                    <select name="idutente" id="idutente"  class="input_moduli sizing float_moduli required" placeholder="Dipendente che ha gestito" title="Dipendente che ha gestito">            
                        <?php
                        $idatelier = $pren[0]['idatelier'];
                        $qry = "select id, nome, cognome from utenti where livello='3' and ruolo in "
                                . "(" . ADDETTO_VENDITE . "," . DISTRICT_MANAGER . "," . FRANCHISING . ", " . STORE_MANAGER . ") " . ($_SESSION['livello'] == 5 || $_SESSION['livello'] == 3 ? ' and attivo=1' : '') . " and (idatelier=$idatelier or id in (select idutente from atelier_utente where idatelier=$idatelier)) order by cognome";
                        $rs = $db->prepare($qry);
                        $rs->execute();
                        $motivono = $rs->fetchAll(PDO::FETCH_ASSOC);
                        echo "<option value=\"\">Seleziona il Dipendente</option>";
                        foreach ($motivono as $motivonod) {
                            echo "<option value=\"" . $motivonod['id'] . "\">" . $motivonod['nome'] . " " . $motivonod['cognome'] . "</option>";
                        }
                        ?>
                    </select>
                <?php } ?>
...
<!--            <select name="sesso" id="sesso" class="extra input_moduli float_moduli">
        ...
        -->
        <div class="boxdx sizing">
            <div class="tit_big">GESTIONE FILE APPUNTAMENTO</div>
            ...
            <div class="imgsx">
                ...
                <div class="caricafile">
                    <a href="javascript:;" id="uploadfile2"> <img src="./immagini/arrow_upload.png" /> Invia file</a>
                    <a href="javascript:;" onclick="javascript:$('.qq-upload-success').remove();" id="cleanupl" style="width: 99px;"><img src="./immagini/clean_upload.png" /> Pulisci Upload</a>
                </div>
            </div>
            <div class="filedx" id="filedx">
                <ul id="elencofiledx">
    ...
    <div class="chiudi"></div>
    <div class="app_passato sizing" style="display: none;">    <!-- controllo prima o dopo appuntamento -->
        <div class="tit_big_bordato sizing">DATI POST APPUNTAMENTO</div>
        <div class="float_primibox float_lft sizing">
            <div class="boxsx sizing">
                <div class="tit_big">DATI ABITO</div>            
                <select name="idtipoabito" id="idtipoabito" onchange="selModabito(this.value, ''), puliscisartoria();" class="input_moduli sizing float_moduli_small_25" placeholder="Tipo di abito" title="Tipo di abito">            
                    <?php
                ...
                <select name="idbollino" id="idbollino" class="input_moduli sizing float_moduli_small_25" placeholder="Bollino" title="Bollino">            
                    <?php
                    $bollino = getDati("bollini", "WHERE idp = 0 ORDER BY id ");
                    echo "<option value=\"\">Seleziona Bollino Abito</option>";
                    foreach ($bollino as $bollinod) {
                        echo "<option value=\"" . $bollinod['id'] . "\">" . $bollinod['nome'] . "</option>";
                    }
                    ?>
                </select>
                <select name="sartoria" id="sartoria" onchange="checksartoria(this.value);" class="input_moduli sizing float_moduli_small_25 required" placeholder="Necessita di sartoria?" title="Necessita di sartoria?">
                    <option value="">Necessita di sartoria?</option>
                    <option value="1">SI</option>
                    <option value="0" selected>NO</option>
                </select>
                <input type="text" name="prezzoabito" id="prezzoabito" class="input_moduli sizing float_moduli_small_25" placeholder="Prezzo abito" onkeyup="saldoAcquisto();" title="Prezzo Abito" />  
            </div>
            <div class="chiudi" style="height: 20px;"></div>
            <div class="boxsx sizing">
        ...
        <div class="chiudi" style="height: 20px;"></div> 
        <div class="box_accessori sizing"> <!-- accessori -->
            <div class="tit_big">ACCESSORI</div>
            <?php
            $accessori = getDati("accessori", "");
            foreach ($accessori as $accessori_d) {
                echo "<div class=\"nomeacc sizing\">{$accessori_d['nome']}</div> "
                . "<input type=\"text\" name=\"accid_[{$accessori_d['id']}]\" id=\"accid_{$accessori_d['id']}\" class=\"prezzoacc input_moduli sizing float_moduli_small\" placeholder=\"Prezzo\" title=\"Prezzo\" onkeyup=\"saldoAcquisto();\" />"
                . "<input type=\"text\" name=\"accnote_[{$accessori_d['id']}]\" id=\"accnote_{$accessori_d['id']}\" class=\"input_moduli sizing float_moduli_50\" placeholder=\"Note\" title=\"Note\" />"
                . "<div class=\"chiudi\"></div>";
            }
            ?>
            <div class="chiudi"></div>
        </div>
        <div class="box_sartoria sizing">
    ...
    <?php } else { ?>
        <div class="bottone_chiudi sizing"><a class="sizing" href="mrgest.php?idatelier=<?= $idatelier ?>">Chiudi</a></div>
    <?php } ?>
...
<script>
    $(function() {
       if($('select#disdetto').length > 0) {
           $('select#disdetto').unbind('change').change(function() {
              var disdetto = $(this).val();
              if(disdetto == '1') {
                  $('select#idutente').removeClass('required');
              } else {
                  $('select#idutente').addClass('required');
              }
           });
       } 
    });
