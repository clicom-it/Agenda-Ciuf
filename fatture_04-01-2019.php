...
switch ($submit) {
    ...
    case "bancapreventivicommessefattureokcliente":
        ...
        $where = "idcliente = $idcliente AND tipo = '0' AND YEAR(data) = '" . DATE('Y') . "' ORDER BY data DESC, numero DESC";
        ...
        die('{"domini" : ' . json_encode($domini) . ', "preventivi" : ' . json_encode($preventivo) . ', "commesse" : ' . json_encode($commesse) . ', "banche" : ' . json_encode($banche) . ', "fatture" : ' . json_encode($fatture) . '}');
        break;
    ...
    case "collegapreventivo":
        ...
        die('{"valori" : ' . json_encode($preventivo) . ', "voci" : ' . json_encode($voci) . ', "ivaselect" : ' . json_encode($ivaselect) . ', "ivadefault" : ' . json_encode($ivadefault) . ', "vociprofit" : ' . json_encode($vociprofit) . '}');
        break;
    ...
    case "collegadominio":
        ...
        die('{"valori" : ' . json_encode($dominio) . ', "ivaselect" : ' . json_encode($ivaselect) . ', "ivadefault" : ' . json_encode($ivadefault) . ', "vociprofit" : ' . json_encode($vociprofit) . '}');
        break;
    ...
    case "collegafattura":
        ...
        die('{"valori" : ' . json_encode($fattura) . ', "voci" : ' . json_encode($voci) . ', "ivaselect" : ' . json_encode($ivaselect) . ', "ivadefault" : ' . json_encode($ivadefault) . ', "vociprofit" : ' . json_encode($vociprofit) . '}');
        break;
    ...
    case "collegacommessa":
        ...
        die('{"valori" : ' . json_encode($commessa) . ', "voci" : ' . json_encode($voci) . ', "ivaselect" : ' . json_encode($ivaselect) . ', "ivadefault" : ' . json_encode($ivadefault) . ', "vociprofit" : ' . json_encode($vociprofit) . '}');
        break;
    ...
    case "editformfattura":
        ...
        /*
         * 
          if ($_POST['idcommfatt'] > 0) {
          for ($c = 0; $c < count($_POST['idcommfatt']); $c++) {
          $sql = $db->prepare("UPDATE commesse SET stato = ? WHERE id = ?");
          $sql->execute(array(3, $_POST['idcommfatt'][$c]));
          }
          }
         * 
         */
...
//            if ($_POST['descrizione'][$i] != "" && $_POST['qta'][$i] != "") {
            ...
            $voci[$i] = array("descrizione" => $_POST['descrizione'][$i], "um" => $_POST['um'][$i], "qta" => $_POST['qta'][$i], "importo" => $_POST['importo'][$i],
                "iva" => $_POST['iva'][$i], "sconto" => $_POST['sconto'][$i], "idprev" => $_POST['idprev'][$i], "idcomm" => $_POST['idcomm'][$i],
...
//        }
        ...
        unset($_POST['submit'], $_POST['descrizione'], $_POST['um'], $_POST['qta'], $_POST['importo'], $_POST['iva'], $_POST['codice_iva'], $_POST['sconto'], $_POST['idprevfatt'], $_POST['idcommfatt'], $_POST['idprev'], $_POST['idcomm'], $_POST['idvocecomm'], $_POST['idfatt'], $_POST['idprofit'], $_POST['datascadenza'], $_POST['importoscadenza'], $_POST['notescadenza'], $_POST['modificascadenze']);
    ...
    case "richiamafattura":
        ...
        die('{"valori" : ' . json_encode($fattura) . ', "voci" : ' . json_encode($voci) . ', "ivaselect" : ' . json_encode($ivaselect) . ', "vociprofit" : ' . json_encode($vociprofit) . ', "scadenze" : ' . json_encode($scadenzeprint) . '}');
        break;
